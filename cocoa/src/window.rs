...
impl Window {
    ...
    /// pointers.
    pub fn new<T: AppDelegate>(
        window_id: usize,
        content_view: ShareId<Object>,
        app_ptr: *const T,
    ) -> Window {
    ...
    pub fn set_dimensions(&mut self, x: f64, y: f64, width: f64, height: f64) {
        unsafe {
            ...
            let _: () = msg_send![&*self.inner, setFrame:dimensions display:YES];
        }
    }
    ...
    pub fn show(&self) {
        unsafe {
            let _: () = msg_send![&*self.inner, makeKeyAndOrderFront: nil];
        }
    }
    ...
    pub fn close(&self) {
        unsafe {
            let _: () = msg_send![&*self.inner, close];
        }
    }
}
...
impl Drop for Window {
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            let _: () = msg_send![&*self.inner, setDelegate: nil];
        }
    }
}
...
extern "C" fn will_close<T: AppDelegate>(this: &Object, _: Sel, _: id) {
    unsafe {
